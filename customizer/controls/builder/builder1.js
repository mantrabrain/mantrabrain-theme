    ...
    var yatriBuilder = {
        ...
        renderData: function () {
            ...
            $('body').on('yatri_render_builder_data', function (event) {
                ...
                console.log(result_node.attr('id'));
        ...
        initDragAndDrop: function () {
            ...
            var drag_drop_accept_selector = ".yatri-builder-sections .item, .yatri-builder-drop-area .grid-content .item";

            var _that = this;

            var droppable_id = 0;

            $.each(_that.$el, function () {

                droppable_id++;

                $(this).find('.yatri-builder-content-wrap').attr('id', 'yatri_drag_and_drop_id_' + droppable_id);

                $('#yatri_drag_and_drop_id_' + droppable_id).attr('data-yatri-droppable-id', droppable_id);

                $('#yatri_drag_and_drop_id_' + droppable_id).find(".yatri-builder-sections .item").addClass('yatri_item_id_' + droppable_id).attr('data-id', droppable_id);

                var accept_selector = '.yatri_item_id_' + droppable_id;

                var draggable_selector = $(accept_selector);

                $('#yatri_drag_and_drop_id_' + droppable_id).find(".yatri-builder-drop-area .grid-content").addClass('yatri_droppable_element_' + droppable_id);

                var droppable_selector = $('.yatri_droppable_element_' + droppable_id);

                _that.initDraggable(draggable_selector, '.yatri_droppable_element_' + droppable_id);
                _that.initDroppable(droppable_selector, accept_selector);
            });

        },
        initDraggable: function (draggable_selector, sortable_selector) {
            ...
            $(draggable_selector).draggable({
                ...
                connectToSortable: sortable_selector,
        ...
        initDroppable: function (droppable_selector, accept_selector) {

            var _that = this;
            $('.yatri-builder-drop-area .item').sortable({
                connectWith: ".yatri-builder-drop-area .item"
            }).disableSelection();
            ...
            droppable_selector.droppable({
                ...
                drop: function (event, ui) {
                    ...
                    //_that.initDragAndDrop();

                    /* setTimeout(function () {
                         droppable_selector.promise().done(function (event, ui) {
                             debugger;
                             $(this).trigger('yatri_render_builder_data');
                         });
                     }, 100);*/
