...
(function ($, wpcustomize) {
    ...
    var yatriBuilder = {
        ...
        initDraggableDropable: function () {
            ...
            $(".yatri-builder-sections .item:not(.disabled), .yatri-builder-drop-area .grid-content .item").draggable({
                helper: "clone",
                revert: "invalid",
                //containment: '.yatri-builder-control',
                ...
                stop: function (ev, ui) {
                    ...
                    if ($(ui.helper).closest('.grid-content').length > 0) {
                        if ($(ui.helper).closest('.grid-content').find('.item').length > 1) {
                            alert('Sorry you cant add more than one item in one grid');
                            $(ui.helper).remove();
                            return;
                        }
                        var key = $(ui.helper).attr('data-section-key');
                        $(ui.helper).closest('.yatri-builder-content-wrap').find(".yatri-builder-sections").find('.item[data-section-key="' + key + '"]').addClass('disabled');
                        $(ui.helper).closest('.yatri-builder-content-wrap').find(".yatri-builder-sections").find('.item.disabled').draggable('disable');
            ...
            $('.yatri-builder-drop-area .grid-content').sortable({

                //connectWith: ".yatri-builder-drop-area .grid-content",
                stop: function (ev, ui) {
                    ...
                    if (ui.helper != null) {
                        ...
                        var grid = $(ui.helper).closest('.grid');

                        _that.moveElement(grid, $(ui.helper));
                    }
                },
            }).disableSelection();
        ...
        moveElement: function (current_grid, dragged_item, loop_count = 0) {
            ...
            if (current_grid.find('.grid-content').find('.item').length > 0) {
        ...
        removeDroppedItem: function () {
            ...
            $('body').on('click', '.yatri-draggable-item .delete-icon', function () {
                var item = $(this).closest('.yatri-draggable-item');
                var key = item.attr("data-section-key");
                item.closest('.yatri-builder-content-wrap').find(".yatri-builder-sections").find('.item[data-section-key="' + key + '"]').draggable('enable').removeClass('disabled');
